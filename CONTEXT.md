...
- **备考经验帖（experience topic）**：**管理端认定**的内容归类（`forum_topics.is_experience`），**不是**学员自述的 `category` 值——学员发帖只能在讨论/提问之间选，无法自称考经。**内容边界 = 围绕考证/备考过程的可复用经验输出**——备考路径规划、资料/课程推荐与评价、考经回忆（考了什么怎么考）、踩坑避坑总结；一次性提问归问答、无主题闲聊归讨论、求职信息不在论坛（判据由管理端掌握，不再依赖发帖者自觉）。**认定经验即蕴含精选**（`is_experience ⇒ is_featured`，库层 CHECK 兜底）：经验区是精选的一个子集，不存在「经验但非精选」。可挂章节也可不挂、不可被采纳；三端第四 Tab（移动端排序三 chip：默认/最多赞/最新；Web 复用 latest/hot 双轴；后端另有 `sort=created` 按发帖时间排，移动端「默认/最新」区分待对接）。激励：发帖计入 growth_post（每日首篇 +10 通用口径）；**「首篇经验专项分」不再独立存在**——原 growth_first_experience（+20 终身一次任务）与精选 +30 合并为同一笔认定奖励，随认定一次性直记（见「精选」）；该任务从 `points_task_config` 删除，**已领取的 +20 不追回、不补差**——未领取者的达成资格随存量帖一律不被认定而自然消失。**存量口径**：迁移前的 `category='experience'` 行一律**不被认定**（降为 `discussion`、`is_experience=false`、不发分、不追回），经验区由管理端从讨论区重新策展。
