...
- **意图与认定（intent vs designation）**：论坛帖的两条正交轴。**意图**=作者自述自己想干什么（`category`：讨论 | 提问），发帖即定；**认定**=管理端授予的帖子事实（`is_featured` 精选位 / `is_experience` 备考经验），学员**不可自述、不可自助获得**。「这帖是考经」「这帖是精选」都是认定而不是意图——**自称不产生事实，也不产生奖励**（原 `category='experience'` 第三值已废止）。
...
- **备考经验帖（experience topic）**：**管理端认定**的内容归类（`forum_topics.is_experience`），**不是**学员自述的 `category` 值——学员发帖只能在讨论/提问之间选，无法自称考经。**内容边界 = 围绕考证/备考过程的可复用经验输出**——备考路径规划、资料/课程推荐与评价、考经回忆（考了什么怎么考）、踩坑避坑总结；一次性提问归问答、无主题闲聊归讨论、求职信息不在论坛（判据由管理端掌握，不再依赖发帖者自觉）。**认定经验即蕴含精选**（`is_experience ⇒ is_featured`，库层 CHECK 兜底）：经验区是精选的一个子集，不存在「经验但非精选」。**认定是叠加标记，不是搬家**——与 `is_featured` 同构，经验帖**仍留在它原本的讨论 Tab**（带经验标识），经验 Tab 只是「已认定考经」的子集视图；讨论/问答 Tab 的 WHERE **不加** `is_experience` 排除条件（改造前 `category='experience'` 天然排除，那是**意图轴**的效果；认定不改意图，故不排除）。可挂章节也可不挂、不可被采纳；三端第四 Tab（移动端排序三 chip：默认/最多赞/最新；Web 复用 latest/hot 双轴；后端另有 `sort=created` 按发帖时间排，移动端「默认/最新」区分待对接）。**列表判据是 `is_experience=true`，不是 `category=experience`**（后者是遗留意图值，存量行已降级、过滤不出行，保留接受只为不给旧客户端 400）。激励：发帖计入 growth_post（每日首篇 +10 通用口径）；**「首篇经验专项分」不再独立存在**——原 growth_first_experience（+20 终身一次任务）与精选 +30 合并为同一笔认定奖励，随认定一次性直记（见「帖子精选」）；该任务从 `points_task_config` 删除，**已领取的 +20 不追回、不补差**——未领取者的达成资格随存量帖一律不被认定而自然消失。**存量口径**：迁移前的 `category='experience'` 行一律**不被认定**（降为 `discussion`、`is_experience=false`、不发分、不追回），经验区由管理端从讨论区重新策展。
- **帖子精选（featured）**：**管理端认定**的帖子级质量信号（`forum_topics.is_featured` 通用列，**全类别可用**、与意图无关），管理端全类别可精/可撤，讨论/问答/经验三 Tab 均有精选筛选与标识展示；**认定奖励 +30** 一次性直记给帖主（每帖幂等一次，取消重精不重复发分，沿用 accepted_bonus 的流水存在判定先例），**与「备考经验」认定共用同一条**（认定经验即蕴含精选，不存在两笔）。**撤精不回滚**——撤精的语义是「不够好」而非「违规」，回滚会让管理员不敢加精；**违规删除才回收**（见「积分回收对冲」）。
