...
- **AI 专业版（AI pro）**：AI 助手里**需解锁才可用**的能力集合（选择模型 / 自定义模型 / 发送图片）。解锁走**积分商城兑换**——按 SKU 记「已拥有」，与课程、真题卷、商城商品同一条 redeem 管线；**不是本地付费标记，也不是页面级的版本态**（不存在「当前正在使用哪个版本」这种状态）。功能宫格与语音输入不属该集合。
