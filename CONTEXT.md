...
- **积分回收对冲（points rollback）**：对已发放流水的追回机制——按 (refType, refID, reasons) 聚合原账取反入账、封底 0、幂等占坑键防双扣，由积分域单实现承载；业务方只声明「回收哪个 ref 的哪些 reason」（如投稿违规下架追回 approved+tier 两笔、论坛违规回收 accepted_bonus），不手写对冲。与「扣罚」的区别：对冲以原账存在为前提、总量不越过已发放部分；扣罚是主动负向入账（ErrInvalidPenalty 守卫）。
...
- **论坛（forum）**：含类别 `category`（`discussion` | `question` | `experience`；`experience`=备考经验帖（#706/#722），可挂章节也可不挂、**不可被采纳**，列表走 `category=experience` 过滤）与采纳状态（`accepted_reply_id`/`solved_at`，问答帖仅楼主可采纳**他人**的一条回答，被采纳回复恒置顶）的图文分离论坛。**图文分离**——主题与回复可携带 `images` 图片 URL 数组（JSONB），正文保持纯文本，不做 markdown 渲染。坐标与意图双维度：`discussion`+NULL=综合讨论区、`discussion`+N=章节讨论区（均为现状）、`question`+NULL=全局问答（新增）、`question`+N=非法（`CHECK (category <> 'question' OR chapter_id IS NULL)` 兜底 + service 400）、`experience`+任意=备考经验帖（可挂章节，值域 CHECK 含三值）。**判类别看 `category`，判区域看 `chapter_id`，两者不可互相替代**——`scope=general` 的定义是 `chapter_id IS NULL`，而问答帖的 `chapter_id` 同样为 NULL，故列表查询必须让 `category` 与 `scope` 共存在同一条 WHERE 里，否则问答帖会整片灌进讨论 Tab（管理端综合区同理）。互动（ADR-0018）：主题点赞（forum_topic_like，幂等，计数经 `likes_count` 反范式列维护，热度排序走索引）、举报（forum_report，待处理/已处理二态，管理端处置沿用删帖/删回复流）、我的帖子/我的回复；问答筛选 `solved`（all/solved/unsolved，仅对 question 有意义）与列表 `reward_issued` 标记。问答帖一律 `chapter_id=NULL`，提问不选章节，章节页不产生问答。**已采纳的帖子只能是问答帖**（#811：`CHECK chk_forum_topics_accepted_requires_question`（迁移 000026）兜底；编辑改类别时 service 拒绝已采纳帖迁移——文案「已采纳的问答帖不能改类别，请先取消采纳」，逃生口是先取消采纳；答主已发的分按「取消采纳不回滚」政策保留）。
- **备考经验帖（experience topic）**：论坛第三类别（#706/#722，`category='experience'`）。**内容边界 = 围绕考证/备考过程的可复用经验输出**——备考路径规划、资料/课程推荐与评价、考经回忆（考了什么怎么考）、踩坑避坑总结；一次性提问归问答、无主题闲聊归讨论、求职信息不在论坛（判据先靠发帖者自觉，治理兜底见「精选」）。可挂章节也可不挂、不可被采纳；三端第四 Tab（移动端排序三 chip：默认/最多赞/最新；Web 复用 latest/hot 双轴；后端另有 `sort=created` 按发帖时间排，移动端「默认/最新」区分待对接）。激励：发帖计入 growth_post（每日首篇 +10 通用口径），另设**首篇经验专项分 +20**（growth 组新任务，终身一次；达成判定需按帖类别过滤，spec 待实现）。
- **精选（featured）**：帖子级别的质量信号（`forum_topics.is_featured` 通用列，全类别可用），管理端全类别可精/可撤，讨论/问答/经验三 Tab 均有精选筛选与标识展示；**加精奖励 featured_bonus +30** 一次性直记给帖主（每帖幂等一次，取消重精不重复发分，沿用 accepted_bonus 的流水存在判定先例）；违规治理仍走既有举报/删帖流（spec 待实现）。
