...
- **论坛（forum）**：含**意图** `category`（仅 `discussion` | `question` 两值，学员自述、只表达作者想干什么）与采纳状态（`accepted_reply_id`/`solved_at`，问答帖仅楼主可采纳**他人**的一条回答，被采纳回复恒置顶——置顶的承载方见下文「回复读取口径」）的图文分离论坛。**图文分离**——主题与回复可携带 `images` 图片 URL 数组（JSONB），正文保持纯文本，不做 markdown 渲染。坐标与意图双维度：`discussion`+NULL=综合讨论区、`discussion`+N=章节讨论区（均为现状）、`question`+NULL=全局问答（新增）、`question`+N=非法（`CHECK (category <> 'question' OR chapter_id IS NULL)` 兜底 + service 400）；类别值域 CHECK 仅两值。**判类别看 `category`，判区域看 `chapter_id`，两者不可互相替代**——`scope=general` 的定义是 `chapter_id IS NULL`，而问答帖的 `chapter_id` 同样为 NULL，故列表查询必须让 `category` 与 `scope` 共存在同一条 WHERE 里，否则问答帖会整片灌进讨论 Tab（管理端综合区同理）。互动（ADR-0018）：主题点赞（forum_topic_like，幂等，计数经 `likes_count` 反范式列维护，热度排序走索引）、举报（forum_report，待处理/已处理二态，管理端处置沿用删帖/删回复流）、我的帖子/我的回复；问答筛选 `solved`（all/solved/unsolved，仅对 question 有意义）与列表 `reward_issued` 标记。**回复读取口径（ADR-0042）**：详情接口 `GET /api/forum/topics/:id` 分页读取回复（`page`/`page_size`，默认 20，响应带 `page`/`pages`/`total`），是回复列表的**唯一读取形态**——旧的「一次性全量返回」已退役，不存在「不传参数即全量」的双语义。**被采纳回复恒置顶由后端承载**：固定占**首页第一条**并从排序结果中剔除（首页容量 = `page_size − 1`，无采纳回复时仍为 `page_size`），置顶优先于 `sort`/`order` 两条排序轴；故采纳通知的深链 `/training/forum/:id#reply-N` 恒命中首页。`reply_count` 口径不变（含楼中楼子树总数，置顶条计入，与 `total` 一致）。**楼中楼维持扁平一层**——`parent_id` 只表达「回复了谁」，不构成可折叠的子树，「昵称 › 被回复人」行内形态所需的名字与头像由详情接口 join 回填（`parent_name`/`parent_avatar_url`）。问答帖一律 `chapter_id=NULL`，提问不选章节，章节页不产生问答。**已采纳的帖子只能是问答帖**（#811：`CHECK chk_forum_topics_accepted_requires_question`（迁移 000026）兜底；编辑改类别时 service 拒绝已采纳帖迁移——文案「已采纳的问答帖不能改类别，请先取消采纳」，逃生口是先取消采纳；答主已发的分按「取消采纳不回滚」政策保留）。
  _Avoid_：「**评论**」——外部产品（即刻等）对同一概念的叫法。本仓库的动作、计数、字段一律称「**回复**」（`forum_replies` / `reply_count` / `parent_name`），界面文案同步；出现「N 条评论」式措辞即术语漂移。
