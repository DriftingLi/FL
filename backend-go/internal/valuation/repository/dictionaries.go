...
func (r *DictionaryRepository) ListVehicleTypes(ctx context.Context) ([]VehicleType, error) {
	rows, err := r.pool.Query(ctx, `SELECT id, name, power_type FROM vehicle_types ORDER BY id ASC`)
	if err != nil {
...
func (r *DictionaryRepository) CreateVehicleType(ctx context.Context, name, powerType string) (VehicleType, error) {
	var id int
	err := r.pool.QueryRow(ctx,
		`INSERT INTO vehicle_types (name, power_type) VALUES ($1, $2) ON CONFLICT (name) DO UPDATE
		 SET power_type = EXCLUDED.power_type RETURNING id`, name, powerType).Scan(&id)
	if err != nil {
		return VehicleType{}, fmt.Errorf("新增车型失败: %w", err)
	}
	return VehicleType{ID: id, Name: name, PowerType: powerType}, nil
}

// UpdateVehicleType 更新车型动力类型
func (r *DictionaryRepository) UpdateVehicleType(ctx context.Context, id int, name, powerType string) error {
	ct, err := r.pool.Exec(ctx,
		`UPDATE vehicle_types SET name = $2, power_type = $3 WHERE id = $1`, id, name, powerType)
	if err != nil {
...
func (r *DictionaryRepository) GetVehicleTypeByName(ctx context.Context, name string) (VehicleType, error) {
	row := r.pool.QueryRow(ctx, `SELECT id, name, power_type FROM vehicle_types WHERE name = $1`, name)
	var v VehicleType
	if err := row.Scan(&v.ID, &v.Name, &v.PowerType); err != nil {
