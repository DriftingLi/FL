...
| 目录 | 入库 | 用途 |
| --- | --- | --- |
| `docs/adr/` | ✅ | 架构决策记录（`ADR-0001-…`，48 篇；移动端另有独立 ADR 体系见 `training-app/…/docs/adr/`） |
...
48 篇，编号 `ADR-0001-…`（如 `ADR-0001-验证码通道适配器seam`、`0028-打卡积分直记化与每日登录事实源迁移`）。清单与最新决策见仓库 `docs/adr/` 目录本身，不在此重复维护。
