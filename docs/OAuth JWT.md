...
1. 登录[扣子编程](https://code.coze.cn/home)。登录 [Coze 平台](https://www.coze.com/home)。
2. 在左侧导航栏选择 **API & SDK。**
3. 在顶部单击**授权** **Authorization**  > **OAuth 应用**页签。
...
| access_deny | app: {app name} is currently deactivated by the owner | * 原因：OAuth 应用已被禁用。 <br> * 解决方案：在扣子 Coze 编程中启用 OAuth 应用。 |
|  | invalid app type | * 原因：应用类型错误。 <br> * 解决方案：渠道应用暂不支持授权码模式。 |
|  | login session invalid | * 原因：登录态无效。 <br> * 解决方案：用户需要重新登录扣子 Coze 编程。 |
