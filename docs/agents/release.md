...
**验收门（合并前置，与上面的 CI/CD 机制是两道独立的门）**：上面第 3–6 步只讲 CI/CD；**PR 正文的 `## 验收证据` 段**由 `.github/workflows/pr-evidence.yml` 对**每一个 PR** 校验，**缺段直接判红**（与改动是否命中运行时面无关）：未命中运行时面写 `免（未命中运行时面）`（先例 #908），命中移动端运行时面则按四门填「执行人 / 日期 / 复测对象 / 结论（含产物）」。四门判据、豁免与「**签收在人、合并不限人**」纪律见 `training-app/叉车维修培训学员端跨端应用/docs/adr/0008-移动端验收门与证据.md`（**写全路径**，根仓库另有一个同名的 `ADR-0008`）。
