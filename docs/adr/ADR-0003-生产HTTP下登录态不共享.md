...
- 状态：已解决（2026-08-18）——原约束前提（生产 443 不可用）已消除，本 ADR 的历史决策仍然正确
- 解决：PR #254 启用 80/443 HTTPS（host 模式 SSL 终止 + 证书由 CD Secrets 注入），`AUTH_COOKIE_SECURE=true` 下 Secure Cookie 恢复可用，跨子域登录态共享按 ADR 预期自动恢复，前端与后端代码无需改动
