...
2. **覆盖审计**：注解侧 197 个端点里，未在 `@Success` 里指认 data DTO 的 17 个已列出（见 spec #940 的验收证据）；跨多域的补齐立后续片，不在本片硬塞。
...
4. **可空性进注解层**：注解层原本不表达 Go 指针语义，试点把 `x-nullable` 扩展落到 `CheckInRankResult.Me`，生成物据此渲染 `T | null` —— 这是「要精确类型就先把事实写进注解」的第一个样例，后续域照此补齐。
