...
**本 ADR 的推迟结论由 ADR-0048 修订为「按域解冻」**：全量 API 层 codegen 仍未开工；片五只交付「可信输入面 + 一个可复制的试点」，而**按域解冻的执行口径（覆盖边界、完成标准、漂移处置、分片顺序）记在 `ADR-0048-前端契约codegen按域解冻与漂移处置`**。本 ADR 原文保留 —— 它记录的推迟理由与「下一次专项以注解为唯一输入」的方向仍然是后续工作的依据。
