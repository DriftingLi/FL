...
- 时间格式：RFC 3339，**业务时区（Asia/Shanghai）墙钟 + 显式偏移 + 微秒定长**，例 `2026-09-11T20:16:52.203824+08:00`。**偏移不可省**（ADR-0043）：客户端有两条消费路径，按 `new Date()` 解析的（Web）与按字符串截取的（移动端取前 16 位），二者都依赖它；发出裸时间戳会让两类客户端全部偏 8 小时
...
**正文子集口径（ADR-0044 / ADR-0052）**：`markdown` 档按**该端声明的子集**渲染 —— 子集**按端分档**（Web / 移动 / 门户各自声明、允许不同，机制也可分道），由该端渲染配置枚举，**名单外的语法在该端不渲染**（记号退回字面量）；**清单不在此处复述**（手抄的清单是第二事实源，子集一变就成假话），口径见 ADR-0044 的「子集补记（#1241）」与 ADR-0052 的「二次 grilling 补记」（Web 侧的显式白名单随 #1253 落地，落地前仍是 `md.disable` 黑名单）。服务端**不按子集校验、不阻断**：越界内容照常入库，只是在该端渲染不出预期形态。客户端工具栏的**按钮集合是该端子集的一个子集**（判据单点 `frontend/src/utils/markdownToolbar.ts`）——按钮集合可以小于子集（公式与 mermaid 在子集内但没有按钮），但按钮承诺的语法必须**在本端预览与发布所用的同一渲染单点上真能渲染**；改子集必须同时看**三处手工同步点**（子集名单 / 按钮表 / 插入语法的 switch）。
