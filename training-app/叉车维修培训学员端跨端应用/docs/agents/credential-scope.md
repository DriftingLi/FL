...
- 公开端点的决策落点在 **ADR-0057「公开端点的证件口径」**（#1125，**在审**）：保持「未传 = 不分区」，显式传参是这些端点唯一的分区通道。
...
| --- | --- | --- | --- | --- |
| `GET /search` | 公开 + `CredentialScoped`（`api/search.go:28`） | query | `api/search.uts` 两出口 ← `pages/search/search.uvue` | 无 JWT ⇒ `roleOf(c)` 恒空 ⇒ 兜底永不生效；不传 = 全证件。**跟随当前** |
| `GET /courses` | 公开 + `CredentialScoped`（`api/courses.go:31` 挂在 courses 蓝图 group，公开路由 `:34`） | query | `api/course.uts#getCourseListApi` ← `pages/courses/courses.uvue`、`pages/mall/mall.uvue` | 同上（#1107 前移动端漏传，页面显示全证件课程） |
| `GET /tags` | **公开、未挂** `CredentialScoped`，handler 自读 query（`api/training_catalog.go:143`） | query | `api/course.uts#getTagsApi` ← `pages/practice/composables/usePracticeOverview.uts` | `question_count` 与**抽题池**同口径（`service.ListQuestionTags` 的 #702 注释）：不传是全证件计数，与学员能抽到的题量对不上（#1107 前移动端漏传） |
...
| 端点 | 移动端消费点 | 为什么不用传 |
| --- | --- | --- |
| `GET /catalog/tree` | `api/course.uts#getCatalogTreeSpecialties`（courses 页筛选、招聘页筛选） | 未挂 `CredentialScoped`、handler 自读 query（`training_catalog.go:105`），但传 `credential_id` **只分区课程节点**；`service.getCatalogTree` 的专业方向 / 等级列表恒为全量启用项（`training_catalog_service.go:456-475`），而移动端**只消费 `specialties`** ⇒ 传了是 no-op。改成消费课程节点时才需要补参数 |
...
- **冻结清单**：`utils/credentialScopeContract.test.js` 扫 `api/`、`pages/`、`composables/` 下的 `credential_id` 传参点，逐文件冻结数量并断言上面三张表的分类与传参点一致。新增传参点、删除传参点、改端点归属都会判红 —— 回来登记本表即可复绿。
...
- **ADR-0057 若改口径**（给 `/search`、`/courses` 挂 `OptionalAuth`，让兜底生效），本表的「必须显式传」两行随之复访 —— 传参点去掉也要回来登记。

## 相关

- 根仓：`docs/adr/ADR-0047-第八波架构深化.md` §4（作用域事实源移到服务端 / 删客户端豁免表）、`docs/adr/ADR-0056-第十一波架构深化.md` §2（两类例外的登记）、`docs/adr/ADR-0057-公开端点的证件口径.md`（在审 #1125）、`CONTEXT.md`「当前证件」三族语义
