...
**状态**：定稿（T02 mall + T03 profile 两模块复盘收束，2026-09-08；决策清单见 `docs/refactor-decisions.md`）
...
- 工单序列：#639（T01 基建）✅ → #640（T02 mall 试点）✅ → #641（T03 profile，PR #684/#696/#698/#699）✅ → **当前：#642（T04 forum，兼 600 预算第三复评）** → #643–#653（T05–T13 手术，dashboard→practice→exam→courses→resume→ai-assistant→auth 三兄弟）→ #652/#653（api 批量收尾，可与主线并行）→ #654（allowlist 退役）
