...
### 决策 ⑧ 的收尾 —— 豁免机制整体退役（#654 / epic #638 T16，2026-09-25）

**本决策 ⑧ 项已被取代（superseded），不是被证伪。** 票 A 的动作是「把一份被 12 处 `indexOf`/`slice`
抠出来的文本收成单一声明点」；#654 的动作是把那份声明点连它承载的**口径**一并删掉 ⇒ ⑧ 的前提
「豁免必须存在，只是不该被复制」不再成立。取舍按 epic 收尾票的票面执行：四条守护规则无豁免全量执法。

现测的删除面（`git show` 可复核）：

- `utils/guardAllowlist.js` 整文件删除（它同时导出名单常量与读者接口 `allowlistPaths()`，两者一起没了）。
  删除前名单里只剩规则 H 两条（`api/checkin.uts` /
  `pages/notifications/notifications.uvue`），而**去豁免跑规则 H 得该套件 54 例全绿**（这是 #654 摘掉
  `exempt.has(...)` 之后自己跑的）⇒ 两条都是过期豁免，摘除零业务改写。#652 替本票预跑的那轮普查
  （「两条命中均为 0」结论即来自它，写在 ADR-0007「既成事实与遗留」节）**用的是字面量 `grep -c`，探针不判别**，
  订正见 ADR-0007 同节 2026-09-25 的「订正」块与「T16」节坑位。
- `utils/modules.js` 的 23 个 `allowlistOwned` 登记位、`utils/contractHarness.js` 的 `allowlistOwnedBy()`
  与两条对账报表键同批删除。
- 17 份模块契约测试的 27 条用例声明（展开 37 例）删除。**为什么删而不是留成空锁**：名单一旦为空，
  `allowlistPaths().filter(...)` 恒返回 `[]`，与「名单里有但没命中」在断言层完全同形 ⇒ 那 37 例从「有牙」
  静默降级成恒真。例外一条：`coursesContract` 同块里「直接对域源文件跑规则 H/I 正则」判的是**产物**、
  不查名单 ⇒ 保留并改名。
- C 组判据从「全仓只有一个声明点」翻成「**零声明点**」。⚠️ **C 组的编号复用是本文件的一个坑**：决策 ⑧ 时代
  的 C1–C5（:144 那张表）测的是「单一声明点 + require 取用 + 恒含两项」，#654 之后同一段落里的是
  **C1–C6**、测的是「零声明点」—— 标签相同、判据相反，读 `it('C2: …')` 时**必须以用例名全文为准**，
  不能按编号回忆。现行六格：C1 扫描面本身非空（防路径断链把「零命中」变成空集合假绿）、C2 全仓代码面
  零声明点（声明它 / 读那份文件 / 抠它的字面量，三支探针各判一侧，外加 `utils/guardAllowlist.js` 文件不存在）、
  C3 三支探针的判别力（注入**真实写法**必红、本文件自用的 `GUARD_` + `ALLOWLIST` 合法分写必不红）、
  C4 五条机械坑位规则体内既无集合式跳过（`.has(`）也无多余的路径式跳过（`.includes('…')`，全表只许 `app-ios`
  一条既有旁路）、C5 两种跳过形态各注入一次必红、C6 登记位 `allowlistOwned` 不含且重注入必抓。
  判据一律走 `codeOnly()`（剥块/行注释后的代码面），历史 ADR 与本仓说明文字还要出现这个名字，
  一条会因为解释自己为什么存在而判红的锁活不过下一次改写。
- **已知残留射程**（写明白而不是假装没有）：① 换名字绕开（另起 `EXEMPT_PATHS`、换一份文件名）C2 的三支
  字面探针与存在性断言都抓不到 —— 本组锁的是**这个机制的名字与形态**，不是「任何形似豁免的东西」；
  ② 规则体内手写单文件常量比较（`if (f === 'x') continue`）不在 C4 判据里。这两格靠每条规则自己的注入自检
  与评审兜；把它们做成文本判据只会得到一条谁都不敢改的正则。
- A8 与 B10/B11（豁免对账三格）随机制摘除，**编号不重排**（与 ⑥ 的「移动不重排」同一条纪律）。
- 决策 ⑧ 的「消费方一律 require 取用」判据同批失效：没有名单可取用了。

③ 门：**134 套 / 2584 例**全绿（术前基线 **134 / 2620**，本票在同一分支 HEAD 上用定点 stash 现测，非引用 #652）。
净减 **36 例**的逐套件对账（两侧都取 `jest --json` 的 `assertionResults.length`）：17 份消费侧契约删掉
27 条用例声明（展开 37 例）= **−37**；`modulesDeclarationContract` **47 → 48 = +1**（用例标题逐条对账：
摘 8 条 = 旧 C1–C5 + A8 + B10/B11，补 9 条 = 新 C1–C6，其中 C3 / C5 是 `it.each` 展开 3 与 2 例）；`utsAndroidCompile` **54 → 54**（规则 H 那条只改名并去掉跳过分支）。
改动与理由全文见 ADR-0007「T16」节。
