...
describe('手术目标页落袋锁（模块全量预算 / 目录 ≤2 层 / 必需文件清单已由声明面执法，#1219）', () => {
  ...
  it('样式块与手术前逐字节一致（UI 像素级冻结的最强证据面：<style> 零 diff）', () => {
    ...
    // 断言宽。#1313 把判据补成真的 sha256，**名字一字未动** —— 它逐字记在
    // `docs/verification/uvue-font-carrier/1269/machine-lines.txt` §7 的红控制凭据里，改名会让那份逐字
    // 记录当场对不上；判据变强不给历史记分，故 §7 那次运行的条数与结论都不受影响。
