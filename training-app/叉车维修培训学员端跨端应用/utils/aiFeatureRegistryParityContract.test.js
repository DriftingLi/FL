 ...
 *
 * 锁三件事，且只锁这三件（**键集合**（原文用词）/ **展示名** / **限免位** `freePreview`）：
 ...
 * 返回 `{ key, label, freePreview }`，键序 = 注册表声明序（本文件不用它，仅供排查时打印）。
 */
function parseChatFeatures(registrySrc) {
  ...
  for (const line of registrySrc.split('\n')) {
    ...
    out.push({ key, label: head[2], freePreview: /freePreview:\s*true/.test(line) });
  }
  return out;
}
...
/** 解析移动端手写清单：`{ key: '…', title: '…', …, featureKey: '…', freePreview: <bool> },` */
function parseMobileItems(constantsSrc) {
  const out = [];
  for (const line of constantsSrc.split('\n')) {
    const m = line.match(/^\s*\{ key: '([^']+)', title: '([^']+)',.*featureKey: '([^']+)', freePreview: (true|false) \},\s*$/);
    if (!m) continue;
    out.push({ localKey: m[1], title: m[2], featureKey: m[3], freePreview: m[4] === 'true' });
  }
  return out;
}
...
describe('功能入口清单 ⟷ 后端注册表 一致性契约（ADR-0009 修订 ② · 交付物 5）', () => {
  describe('① 解析器自身有牙（先证明检测器不空跑）', () => {
    ...
    it('清单解析到 4 项，且键/展示名/限免位都取到了（不是空跑）', () => {
  ...
  describe('② 真实文件：键集合 / 展示名 / 限免位 三件全等', () => {
    it('三项契约全等（唯一的综合判据）', () => {
      expect(parityIssues(MOBILE_ITEMS, CHAT_FEATURES)).toEqual([]);
    });
