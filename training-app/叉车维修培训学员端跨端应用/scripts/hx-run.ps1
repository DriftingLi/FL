...
.DESCRIPTION
    ...
    **真运行会话常驻（2026-09-13 实测，勿删）**：不传 `--compile` 的**真运行** `cli.exe` **不会自己返回**
    （实测 13:59 发起，26 分钟后仍活着，且每约 10 分钟重试一次 `wakeUpDevice`；那次重试被设备以
    `INJECT_EVENTS` 拒绝，**非致命**，部署照常完成）。只有「仅编译」调用才会自己收口。
    ⇒ 本脚本对 launch 步**不等待进程退出**：发出去 → **有界轮询设备侧事实**（`-TimeoutSeconds`，默认 1800 秒）→
    判完即返回，**既不等待也不终止**那个会话（要停就由人在 HBuilderX 里点停止）。
    「上一次的运行会话还在」时再发起一次会怎样 —— **未实测**，故不在这里假装有保护。
    部署耗时实测：冷启 HBuilderX + 增量编译约 610 秒，资源导出与推送另约 **9 分钟**（13:59 发起、14:10 落盘）。
