...
| 层 | 触发 | 动作 | 成本 |
| --- | --- | --- | --- |
| **内循环（只编译拿诊断）** | 改完 `.uvue` / `.uts`，**先确认没有编译期诊断** | `npm run hx:compile-only`（= `hx-run.ps1 -CompileOnly`：官方语义的「仅编译」，**不推送 / 不启动 / 不轮询 / 不需要设备**，机检行 `mode=compile-only`） | 约 3–4 分钟（≈ 编译段；实测编译 205–261 秒） |
...
- **在真机上验「本该编译期就红」的东西**——类型名义不一致（`ClassCastException`）、uvue 样式规则违反、模板编译错误**都是编译期诊断**：先 `npm run hx:compile-only`（约 3 分钟、**不碰设备**），别让它变成一次 5 分钟的真机返工。2026-09-13 实测就这样白烧了两次。
