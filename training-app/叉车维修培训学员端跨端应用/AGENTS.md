...
> `scripts/hx-run.ps1`（`npm run hx:run`）是**日常运行到真机**（增量编译 + 资源推送到设备，实测一轮约 10–11 分钟），**不是门、不进 `## 验收证据`**。
>
> **`--compile` 的语义坑位（2026-09-13，血账）**：HBuilderX 官方帮助里 `--compile` 是「**仅编译代码**」（默认 false），
> 不是「先编译再运行」。所以 **`--compile true` 只属于全量编译门 `compile-check.ps1`**；`hx-run.ps1` 从不传它
> —— 从前它照抄了门的调用，于是**从未真正请求过运行**，`编译成功 → 已停止运行...` 被误读成「运行失败」并追了整整一票（#917）。
...
| **内循环（增量运行）** | 改 `<script>` 结构 / 新增文件（热刷新不生效） | `npm run hx:run`（= `scripts/hx-run.ps1`，**真运行**：无干净缓存重建、不重装基座、不做 `adb install`；判定靠**设备侧事实相对基线前进**） | 约 10–11 分钟 |
...
**时间花在哪（2026-09-13 实测校准）**：基座 APK（95.7 MB）**只装一次**；冷启 HBuilderX + 增量编译约 **610 秒**，
**资源导出与推送另约 9 分钟**（13:59 发起、14:10 设备侧落盘），一轮真运行合计约 **10–11 分钟**；
全量编译 **4.8–8 分钟**。旧口径写的「增量约 1–2 分钟」**只覆盖编译段**，是把「仅编译」的耗时当成了「运行」的耗时，勿再引用。
`hx-run.ps1` 会打印分段表与机检行 `HX_RUN mode=… compile=… deploy=… total=… exit=…`，另加部署判据行 `HX_RUN_DEPLOY deployed=… www=… pid_after=…`。

**真运行会话常驻（2026-09-13 实测）**：不传 `--compile` 的真运行 `cli.exe` **不会自己收口**（实测发起后 26 分钟仍活着，
每约 10 分钟重试一次 `wakeUpDevice`，被设备以 `INJECT_EVENTS` 拒绝、**非致命**）。所以 `hx-run.ps1` 对它
**不等待**：发出去 → 有界轮询设备侧事实 → 判完即返回；要停那个会话就在 HBuilderX 里点「停止」（脚本绝不 kill）。
