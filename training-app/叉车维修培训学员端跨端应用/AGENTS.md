...
| 层 | 触发 | 动作 | 成本 |
| --- | --- | --- | --- |
| **内循环（只编译拿诊断）** | 改完 `.uvue` / `.uts`，**先确认没有编译期诊断** | `npm run hx:compile-only`（= `hx-run.ps1 -CompileOnly`：官方语义的「仅编译」，**不推送 / 不启动 / 不轮询 / 不需要设备**，机检行 `mode=compile-only`） | **热缓存** 205–261 秒；**冷 / 失效缓存 >901 秒**（2026-09-14 实测被 900 秒超时切断，**勿再引用「约 3–4 分钟」当常态**）|
