<template>
  <div class="ai-chat-shell relative flex h-screen overflow-hidden bg-canvas">
    ...
    <!-- 右侧对话区（空态：欢迎区贴顶、输入框紧跟其下落在视口中部；有消息：列表占满、输入框沉底） -->
    <main class="chat-main flex min-w-0 flex-1 flex-col bg-panel" :class="isWelcome ? 'overflow-y-auto' : 'overflow-hidden'">
      <!-- 消息列表 -->
      <div
        ref="messageListRef"
      ...
      >
        <!-- 空状态：欢迎区（图标左标题右横排；标题即一句话功能介绍，无副标题；
             欢迎区贴顶上移、输入框紧跟其后：居中由「紧凑欢迎区+输入框」的堆叠高度决定，
             不用 auto margin 对劈剩余空间（会把输入框推向下方视口底） -->
        <div v-if="isWelcome" class="welcome-area w-full px-6 pb-1 pt-8 text-center">
      ...
      <!-- 输入区（空态：紧跟欢迎区落在视口中部；有消息：沉底 dock） -->
      <div
