...
// 电动叉车参数录入（Tesla 极简风：扁平、白底、4px 圆角）
...
import { WORK_CONDITION_OPTIONS } from '@/utils/valuationConstants'
...
const { form, itemStatusMap, filledCount, totalCount, submitting, setAllNormal, reset, submit } =
  useEvaluationForm('electric')

const { data: configs, loading: loadingConfigs, load: loadConfigs } = usePartConfigs('electric')
const brands = ref<Brand[]>([])

/** 选中品牌的常见型号列表；切品牌时清空已选型号 */
...
<template>
  <div class="app-container input-page valuation-root">
    <PageHeader
      title="电动叉车参数录入"
      subtitle="electric forklift · parameter input"
    >
      <template #actions>
