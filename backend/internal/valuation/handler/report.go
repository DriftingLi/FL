...
// resolver 用于重建建议时动态读取 coefficient_configs（与评估流程同一份配置）。
func NewReportHandler(evalRepo *repository.EvaluationRepository, gen *pdf.Generator, l *zap.Logger, st storage.Storage, resolver service.ConfigResolver) *ReportHandler {
	return &ReportHandler{evalRepo: evalRepo, generator: gen, storage: st, logger: l, resolver: resolver}
}
...
func (h *ReportHandler) Generate(c *gin.Context) {
	...
	dimScores, suggestions := h.rebuildDerivedFields(c.Request.Context(), detail)
...
func (h *ReportHandler) regenerateAndUpload(ctx context.Context, detail *model.EvaluationDetail, id int64) string {
	dimScores, suggestions := h.rebuildDerivedFields(ctx, detail)
...
// 建议走 service.BuildSuggestions（与评估流程同一实现），
// 百分比通过 resolver 动态读取 coefficient_configs，避免硬编码导致建议分叉。
func (h *ReportHandler) rebuildDerivedFields(ctx context.Context, d *model.EvaluationDetail) (map[string]float64, []string) {
	if d == nil {
		return nil, nil
	}
	...
	suggestions := service.BuildSuggestions(ctx, service.FromDetail(d), h.resolver)
	if suggestions == nil {
		suggestions = []string{}
	}
