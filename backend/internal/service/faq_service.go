...
type FaqCategoryDTO struct {
	...
	Entries   []FaqEntryDTO `json:"entries" nullability:"nonnil"`
	ID        int           `json:"id"`
	SortOrder int           `json:"sort_order"`
...
type FaqResult struct {
	Categories []FaqCategoryDTO `json:"categories" nullability:"nonnil"`
}
...
type AdminFaqCategoriesResult struct {
	Categories []AdminFaqCategoryDTO `json:"categories" nullability:"nonnil"`
}

// AdminFaqEntriesResult 管理端条目清单载荷。
type AdminFaqEntriesResult struct {
	Entries []AdminFaqEntryDTO `json:"entries" nullability:"nonnil"`
}
