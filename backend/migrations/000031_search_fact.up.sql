...
--
-- 只有关键词 / 分区类型 / 命中数 / 时间四列，且**刻意不设** user_id、credential_id、ip、device：
...
CREATE TABLE IF NOT EXISTS search_fact (
    ...
    total_hits  INTEGER      NOT NULL DEFAULT 0,   -- 该次搜索的总命中数（0 = 零结果）
...
-- 零结果词聚合：按 total_hits=0 + 时间窗扫，再按关键词分组。
CREATE INDEX IF NOT EXISTS idx_search_fact_zero ON search_fact (total_hits, created_at);
